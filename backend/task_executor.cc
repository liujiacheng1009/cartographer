...
void Task::SetTaskExecutor(TaskExecutor* task_executor) {
  absl::MutexLock locker(&mutex_);
  CHECK_EQ(state_, NEW);
  state_ = DISPATCHED;
  task_executor_to_notify_ = task_executor;
  if (uncompleted_dependencies_ == 0) {
    state_ = DEPENDENCIES_COMPLETED;
    CHECK(task_executor_to_notify_);
    task_executor_to_notify_->NotifyDependenciesCompleted(this);
  }
}
...
void TaskExecutor::Execute(Task* task) { task->Execute(); }

void TaskExecutor::SetTaskExecutor(Task* task) {
  task->SetTaskExecutor(this);
}

TaskExecutor::TaskExecutor(int num_threads) {
  CHECK_GT(num_threads, 0) << "TaskExecutor requires a positive num_threads!";
  absl::MutexLock locker(&mutex_);
  for (int i = 0; i != num_threads; ++i) {
    pool_.emplace_back([this]() { TaskExecutor::DoWork(); });
  }
}
...
void TaskExecutor::NotifyDependenciesCompleted(Task* task) {
  absl::MutexLock locker(&mutex_);
...
std::weak_ptr<Task> TaskExecutor::Schedule(std::unique_ptr<Task> task) {
  ...
  SetTaskExecutor(shared_task.get());
  return shared_task;
}
