...
DEFINE_string(trajectory_filename, "",
              "If non-empty, writes optimized trajectory nodes as CSV.");
...
void RunOfflineNode(const MapBuilderFactory& map_builder_factory,
  ...
  if (!FLAGS_trajectory_filename.empty()) {
    CHECK_EQ(bag_index_to_trajectory_id.size(), 1u)
        << "CSV trajectory export currently requires exactly one bag.";
    node.WriteTrajectoryCsv(FLAGS_trajectory_filename,
                            bag_index_to_trajectory_id.begin()->second);
  }
