...
TrajectoryOptions CreateTrajectoryOptions(
  ...
  options.trajectory_builder_options =
      ::cartographer::mapping::CreateTrajectoryBuilderOptions(
          parameter_dictionary->GetDictionary("trajectory_builder").get());
  options.tracking_frame =
      parameter_dictionary->GetString("tracking_frame");
  options.published_frame =
      parameter_dictionary->GetString("published_frame");
  options.odom_frame = parameter_dictionary->GetString("odom_frame");
  options.provide_odom_frame =
      parameter_dictionary->GetBool("provide_odom_frame");
  options.use_odometry = parameter_dictionary->GetBool("use_odometry");
  options.use_nav_sat = parameter_dictionary->GetBool("use_nav_sat");
  options.use_landmarks = parameter_dictionary->GetBool("use_landmarks");
  options.publish_frame_projected_to_2d =
      parameter_dictionary->GetBool("publish_frame_projected_to_2d");
  options.num_laser_scans =
      parameter_dictionary->GetNonNegativeInt("num_laser_scans");
  options.num_multi_echo_laser_scans =
      parameter_dictionary->GetNonNegativeInt("num_multi_echo_laser_scans");
  options.num_subdivisions_per_laser_scan =
      parameter_dictionary->GetNonNegativeInt(
          "num_subdivisions_per_laser_scan");
  options.num_point_clouds =
      parameter_dictionary->GetNonNegativeInt("num_point_clouds");
  ...
  options.fixed_frame_pose_sampling_ratio =
      parameter_dictionary->GetDouble("fixed_frame_pose_sampling_ratio");
  options.imu_sampling_ratio =
      parameter_dictionary->GetDouble("imu_sampling_ratio");
  options.landmarks_sampling_ratio =
      parameter_dictionary->GetDouble("landmarks_sampling_ratio");
