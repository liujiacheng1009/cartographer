    ...
    : node_options_(node_options)
{
  ...
  run_final_optimization_server_ =
      node_->create_service<cartographer_ros_msgs::srv::RunFinalOptimization>(
          kRunFinalOptimizationServiceName,
          std::bind(&Node::handleRunFinalOptimization, this,
...
void Node::WriteTrajectoryCsv(const std::string& filename, int trajectory_id) {
  absl::MutexLock lock(&mutex_);
  auto request = std::make_shared<cartographer_ros_msgs::srv::TrajectoryQuery::Request>();
  auto response = std::make_shared<cartographer_ros_msgs::srv::TrajectoryQuery::Response>();
  ...
  output << "timestamp,x,y,theta\n";
  for (const auto& stamped : response->trajectory) {
    const auto& q = stamped.pose.orientation;
    const double yaw = std::atan2(2. * (q.w * q.z + q.x * q.y),
                                  1. - 2. * (q.y * q.y + q.z * q.z));
    const double timestamp = static_cast<double>(stamped.header.stamp.sec) +
                             1e-9 * stamped.header.stamp.nanosec;
    output << timestamp << ',' << stamped.pose.position.x << ','
           << stamped.pose.position.y << ',' << yaw << '\n';
  }
}
...
bool Node::ValidateTrajectoryOptions(const TrajectoryOptions& options) {
  return node_options_.map_builder_options.use_trajectory_builder_2d() &&
...
bool Node::handleRunFinalOptimization(
    const cartographer_ros_msgs::srv::RunFinalOptimization::Request::SharedPtr,
    cartographer_ros_msgs::srv::RunFinalOptimization::Response::SharedPtr response) {
  RunFinalOptimization();
  response->status.code = cartographer_ros_msgs::msg::StatusCode::OK;
  response->status.message = "Final optimization completed.";
  return true;
}
