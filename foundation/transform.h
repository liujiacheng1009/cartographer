...
Sophus::SE2<T> MakeRigid2(const Eigen::Matrix<T, 2, 1>& translation,
                          const T& yaw) { return Sophus::SE2<T>(yaw, translation); }

template <typename Derived>
Sophus::SE2<typename Derived::Scalar> MakeRigid2Translation(
    const Eigen::MatrixBase<Derived>& translation) {
  return Sophus::SE2<typename Derived::Scalar>::trans(translation);
}

template <typename T>
T Yaw(const Sophus::SE2<T>& transform) { return transform.so2().log(); }

template <typename T>
Sophus::SE3<T> Embed3D(const Sophus::SE2<T>& transform) {
  const Eigen::Matrix<T, 3, 1> t(transform.translation().x(), transform.translation().y(), T(0));
  return Sophus::SE3<T>(Sophus::SO3<T>(Eigen::Quaternion<T>(Eigen::AngleAxis<T>(
      transform.so2().log(), Eigen::Matrix<T, 3, 1>::UnitZ()))), t);
}

template <typename T>
Sophus::SE3<T> MakeRigid3Rotation(const Eigen::AngleAxis<T>& rotation) {
  return Sophus::SE3<T>(Sophus::SO3<T>(Eigen::Quaternion<T>(rotation)),
